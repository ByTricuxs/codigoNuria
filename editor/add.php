<?php 
    ...
    if($_POST){ // Verifica si el formulario envía datos mediante POST.
        date_default_timezone_set('America/Costa_Rica'); // Establece la zona horaria para registrar las fechas correctamente.
        
        $database->insert("tb_game_config",[ // Inserta un nuevo registro en la tabla tb_game_config.
            "game_data"=>$_POST['data'], // Almacena el contenido del campo 'data' en 'game_data'.
            "created_at"=>date("Y-m-d H:i:s"), // Establece la fecha y hora de creación.
            "updated_at"=>date("Y-m-d H:i:s") // Establece la fecha y hora de actualización (misma que la de creación en este caso).
        ]);

        header("Location: ./index.php"); // Redirige a index.php una vez completada la inserción.
    }
?>
...
<body>
    ...
    <form action="./add.php" method="POST"> // Formulario que envía datos a add.php mediante el método POST.
        <textarea id="textarea1" wrap="soft" spellcheck="false" name="data"></textarea> // Campo de texto para ingresar JSON o configuraciones de juego.
        <input type="submit" value="Submit data" style="display:block; position: absolute; with: 15%; bottom: 15px; background: #a5a5a5; 
        color: #1b1b1b; font-size: 18px; text-align: center; height: 30px; z-index: 10; right: 40px;"> // Botón para enviar los datos.
    </form>
</div>
